...
/**
 * AsyncTask-Klasse: Diese Klasse ermöglicht es mehrere Links auf einmal zu
 * setzen. Die ObjectEntities sind als HashSet zu übergeben. BACKEND-ENDPUNKT:
 ...
 *            ableitet
 */
public class PostLinksFromSameType<T extends Entity> extends
		AsyncTask<Void, Void, Void> {
	
	private String				errorMessage	= Constants.NO_ERROR;
	private String				saveOkMessage	= Constants.SAVING_OF_SEVERAL_ENTITIES_OK;
	...
	public PostLinksFromSameType(Link link, T entityOne,
	...
	private void formatLink(T objectEntity) {
		...
		saveOkMessage = saveOkMessage + " " + objectEntity.getId();
	}
	...
	private void setPostURL(T objectEntity) throws UnsupportedEncodingException {
		...
		postUri = Constants.BACKEND_URI
				...
				+ Constants.ENTITIES + "/" + entityOne.getId() + "/"
				+ Constants.LINKS + "/"
				+ URLEncoder.encode(link.getType(), Constants.UTF_STRING) + "/"
				+ objectEntity.getId();
	}
	
	private boolean checkObjectEntity(T objectEntity) {
	...
	private boolean checkParameters() {
		...
		return entityOne != null && objectEntities != null
				...
				&& link != null && !link.getType().isEmpty() && appName != null
				&& !appName.isEmpty() && apiKey != null && !apiKey.isEmpty()
				&& entityOne.getType() > 0 && link.getProperties().size() > 0;
	}
